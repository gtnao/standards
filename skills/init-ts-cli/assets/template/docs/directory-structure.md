...
実行方法に応じた入力の解析・検証、[Zodによる環境変数の検証](../AGENTS.md#environment)、adapterの生成を行い、usecaseへ`deps`と`input`を渡す。
...
この結果や、必要に応じて依存への呼び出しを検証する。テストの配置は[テスト方針](../AGENTS.md#testing)に従う。
